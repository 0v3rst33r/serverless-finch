 ...
 */
function uploadDirectory(aws, bucketName, clientRoot, headerSpec, orderSpec, keyPrefix, sse, contentEncoding) {
  ...
  return filesGroupedByOrder.reduce((existingUploads, files) => {
    return existingUploads.then(existingResults => {
      ...
      return Promise.all(
        uploadList.map(u => uploadFile(aws, bucketName, u.filePath, u.fileKey, u.headers, sse, contentEncoding))
 ...
 * @param {string} sse - ServerSideEncryption, AES256 | aws:kms | null
 */
function uploadFile(aws, bucketName, filePath, fileKey, headers, sse, contentEncoding) {
  ...
  const params = {
    ...
    Key: fileKey,
    Body: fileBuffer,
    ...getMimeTypeAndContentEncoding(filePath, contentEncoding)
  };
...
function getMimeTypeAndContentEncoding(filePath, contentEncoding) {
  if (contentEncoding) {
    return {
